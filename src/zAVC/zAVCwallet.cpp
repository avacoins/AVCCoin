...
CzAVCWallet::CzAVCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zAVC seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAVCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAVCSeed_deprecated()) {
                    LogPrintf("%s: Updated zAVC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zAVC seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zAVC wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAVCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAVCCount(nCountLastUsed);
    else if (!walletdb.ReadZAVCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAVCWallet::Lock()
{
    seedMaster = 0;
}

void CzAVCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAVCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAVC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAVCWallet::LoadMintPoolFromDB()
{
...
void CzAVCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAVCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAVCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
void CzAVCWallet::SeedToZAVC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAVCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAVCWallet::GenerateDeterministicZAVC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAVCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAVCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAVCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
