...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 AVC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAVC send
enum ZerocoinSpendStatus {
    ZAVC_SPEND_OKAY = 0,                            // No error
    ZAVC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAVC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAVC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAVC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAVC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAVC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAVC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAVC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAVC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAVC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAVC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAVC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAVC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAVC_SPENT_USED_ZAVC = 14,                      // Coin has already been spend
    ZAVC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZAVC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZAVCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);

    std::string GetUniqueWalletBackupName(bool fzAVCAuto) const;
    ...
    */
    ...
    /// Get 10000 AVC output and keys which can be used for the Masternode
