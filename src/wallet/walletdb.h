...
public:
    ...
    bool WriteZAVCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZAVCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZAVCSeed_deprecated(uint256& seed);
    bool EraseZAVCSeed();
    bool EraseZAVCSeed_deprecated();

    bool WriteZAVCCount(const uint32_t& nCount);
    bool ReadZAVCCount(uint32_t& nCount);
