...
CZAVCStake::CZAVCStake(const libzerocoin::CoinSpend& spend)
{
...
int CZAVCStake::GetChecksumHeightFromMint()
{
...
int CZAVCStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZAVCStake::GetChecksum()
{
    return nChecksum;
}

// The zAVC block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZAVCStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZAVCStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zAVC staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZAVCStake::GetUniqueness()
{
    //The unique identifier for a zAVC is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZAVCStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZAVCStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zAVC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAVCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAVC output", __func__);
    vout.emplace_back(outReward);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAVCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAVC output", __func__);
...
//!AVC Stake
bool CAVCStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAVCStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CAVCStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAVCStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CAVCStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CAVCStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CAVCStake::GetUniqueness()
{
    //The unique identifier for a AVC stake is the outpoint
...
CBlockIndex* CAVCStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
