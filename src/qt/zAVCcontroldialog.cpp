...
#include "zAVCcontroldialog.h"
#include "ui_zAVCcontroldialog.h"
...
std::set<std::string> ZAVCControlDialog::setSelectedMints;
std::set<CMintMeta> ZAVCControlDialog::setMints;

bool CZAVCControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZAVCControlDialog::COLUMN_DENOMINATION || column == ZAVCControlDialog::COLUMN_VERSION || column == ZAVCControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZAVCControlDialog::ZAVCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZAVCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zAVC Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZAVC->setProperty("cssClass", "text-main-purple");
    ui->labelZAVC_int->setProperty("cssClass", "text-main-purple");
...
ZAVCControlDialog::~ZAVCControlDialog()
{
    delete ui;
}

void ZAVCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAVCControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZAVCControlWidgetItem* itemDenom(new CZAVCControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZAVCControlWidgetItem *itemMint = new CZAVCControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zAVC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAVC seed used to mint this zAVC is not the same as currently hold in the wallet";
            else
...
void ZAVCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAVCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAVC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZAVCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAVCControlDialog::GetSelectedMints()
{
...
void ZAVCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
