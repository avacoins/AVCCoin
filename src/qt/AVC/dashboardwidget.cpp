...
#include "qt/AVC/dashboardwidget.h"
#include "qt/AVC/forms/ui_dashboardwidget.h"
#include "qt/AVC/sendconfirmdialog.h"
#include "qt/AVC/txrow.h"
...
DashboardWidget::DashboardWidget(AVCGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of AVC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareAVC, "square-chart-AVC");
    setCssProperty(ui->labelAVC, "text-chart-AVC");
    ...
    ui->labelAmountAVC->setText("0 AVC");
    setCssProperty(ui->labelAmountAVC, "text-stake-AVC-disable");
    ...
    ui->btnHowTo->setText(tr("How to get AVC"));
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            AVC = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalAVC += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAVC.append(AVC);

        int max = std::max(AVC, AVC);
...
void DashboardWidget::onChartRefreshed() {
    ...
    set0->append(chartData->valuesAVC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAVC > 0) {
        setCssProperty(ui->labelAmountAVC, "text-stake-AVC");
    } else {
        setCssProperty(ui->labelAmountAVC, "text-stake-AVC-disable");
    }
    forceUpdateStyle({ui->labelAmountAVC});
    ui->labelAmountAVC->setText(GUIUtil::formatBalance(chartData->totalAVC, nDisplayUnit));
