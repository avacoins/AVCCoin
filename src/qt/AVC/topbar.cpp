...
#include "qt/AVC/topbar.h"
#include "qt/AVC/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/AVC/lockunlock.h"
#include "qt/AVC/qtutils.h"
#include "qt/AVC/receivedialog.h"
...
TopBar::TopBar(AVCGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopAVC}, "amount-small-topbar");
    setCssProperty(ui->labelAmountAVC, "amount-topbar");
    setCssProperty({ui->labelPendingAVC, ui->labelImmatureAVC }, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // AVC Total
    CAmount AVCAvailableBalance = balance;

    // Set
    QString totalAVC = GUIUtil::formatBalance(AVCAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopAVC->setText(totalAVC);

    // Expanded
    ui->labelAmountAVC->setText(totalAVC);

    ui->labelPendingAVC->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureAVC->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
