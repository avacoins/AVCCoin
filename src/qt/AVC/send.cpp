...
#include "qt/AVC/send.h"
#include "qt/AVC/forms/ui_send.h"
#include "qt/AVC/addnewcontactdialog.h"
#include "qt/AVC/qtutils.h"
#include "qt/AVC/sendchangeaddressdialog.h"
#include "qt/AVC/optionbutton.h"
#include "qt/AVC/sendconfirmdialog.h"
#include "qt/AVC/myaddressrow.h"
#include "qt/AVC/guitransactionsutils.h"
...
SendWidget::SendWidget(AVCGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (AVC)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a AVC address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 AVC");
	...
	setCssProperty(coinIcon, "coin-icon-AVC");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_AVC, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any AVC to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress AVCAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(AVCAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(AVCAdd.Get(), dialog->getLabel().toStdString(),
				AddressBook::AddressBookPurpose::SEND)) {
