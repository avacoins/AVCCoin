...
#include "qt/AVC/forms/ui_addresseswidget.h"
#include "qt/AVC/addresslabelrow.h"
#include "qt/AVC/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(AVCGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a AVC address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress AVCAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(AVCAdd.Get(), label.toUtf8().constData(),
                AVCAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
