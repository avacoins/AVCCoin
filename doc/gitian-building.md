...
*Setup instructions for a gitian build of AVC using a VM or physical system.*

Gitian is the deterministic build process that is used to build the AVC
...
to the AVC GitHub release page.
...
- [Building AVC](#building-AVC-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/AVC-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/AVC-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/AVC-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/AVC-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/AVC-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/AVC-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/AVC-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@AVC.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/AVC-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/AVC-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/AVC-osx-*-build.assert
```
